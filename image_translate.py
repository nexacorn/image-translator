...
def expand_bbox(image, bbox, max_expand=10):
    """
...
def translate_llm(original_text_len, orginal_text):
    ...
    #         {
    #             "role": "system",
    #             "content": "배열로 들어온 총 " + str(len(original_texts)) + "개의 유저 메시지는 패션상품의 광고문구야 무조건 빠짐없이 모든 메시지들을 번역해야돼. 즉, "+str(len(original_texts))+"개의 번역결과가 나와야해. 번역이 잘 안되는 것들은 그대로 순서에 맞게 반환해줘. 이것들을 패션 상품에 어울리는 한글로, 가능한 개조식으로 번역해줘. 배열 형식과 순서는 유지한채 json 형식으로, 한글로 번역해서 반환해줘. return example : { translated : [\"번역 결과\",\"번역 결과\",\"번역 결과\",\"번역 결과\"] }",
    #         },
    #         {"role": "user", "content": original_texts_string},
    #     ],
    # )
    completion = client.chat.completions.create(
        model="gpt-3.5-turbo-1106",
...
def image_translate(predict_config):
    ...
    for key_folder in key_folders:
        ...
        for input_image in tqdm.tqdm(images):
            ...
            mask_img = resize_max_size(mask_img, size_limit=size_limit, interpolation=interpolation)
            
            inpainted_result = lama_model(img, mask_img, config)
            inpainted_result = cv2.cvtColor(inpainted_result.astype(np.uint8), cv2.COLOR_BGR2RGB)
            if alpha_channel is not None:
                if alpha_channel.shape[:2] != inpainted_result.shape[:2]:
                    alpha_channel = cv2.resize(
                        alpha_channel, dsize=(inpainted_result.shape[1], inpainted_result.shape[0])
                    )
            ...
            completion = translate_llm(len(original_texts), original_texts_string)
